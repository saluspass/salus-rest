...
bool CivetWebSocketHandler::handleData(CivetServer* server, struct mg_connection* conn, int bits, char* data, size_t data_len)
{
	UNUSED_PARAMETER(server);
	UNUSED_PARAMETER(conn);
...
int CivetServer::requestHandler(struct mg_connection* conn, void* cbdata)
{
	...
	CivetServer* me = reinterpret_cast<CivetServer*>(request_info->user_data);
	assert(me != nullptr);
	...
	CivetHandler* handler = reinterpret_cast<CivetHandler*>(cbdata);
...
int CivetServer::authHandler(struct mg_connection* conn, void* cbdata)
{
	...
	CivetAuthHandler* handler = reinterpret_cast<CivetAuthHandler*>(cbdata);
...
int CivetServer::webSocketConnectionHandler(const struct mg_connection* conn, void* cbdata)
{
	...
	CivetWebSocketHandler* handler = reinterpret_cast<CivetWebSocketHandler*>(cbdata);
...
CivetServer::CivetServer(std::vector<std::string> options, const struct CivetCallbacks *_callbacks, const void* UserContextIn)
    : context(0)
{
	struct CivetCallbacks callbacks;
...
{
	mg_set_websocket_handler(context, uri.c_str(), webSocketConnectionHandler, webSocketReadyHandler, webSocketDataHandler, webSocketCloseHandler, handler);
}
...
bool CivetServer::getParam(struct mg_connection* conn, const char* name, std::string& dst, size_t occurrence)
{
	...
	assert(ri != nullptr);
	CivetServer* me = reinterpret_cast<CivetServer*>(ri->user_data);
	assert(me != nullptr);
	mg_lock_context(me->context);
	...
	else 
	{
		...
		if (con_len_str) 
		{
			...
			if (con_len > 0) 
			{
				...
				conobj.postData = reinterpret_cast<char*>(malloc(con_len + 1));
				if (conobj.postData != NULL) 
				{
...
bool CivetServer::getParam(const char* data, size_t data_len, const char* name, std::string& dst, size_t occurrence)
{
	...
	for (const char* p = data; p + name_len < e; p++) 
	{
		if ((p == data || p[-1] == '&') && p[name_len] == '=' && !mg_strncasecmp(name, p, name_len) && 0 == occurrence--)
		{
			...
			s = reinterpret_cast<const char*>(memchr(p, '&', (size_t)(e - p)));
			if (s == nullptr) 
			{
				s = e;
			}
...
std::vector<int> CivetServer::getListeningPorts()
{
	...
	int size = mg_get_server_ports(context, (int)server_ports.size(), &server_ports[0]);
	if (size <= 0) 
	{
		ports.resize(0);
		return ports;
